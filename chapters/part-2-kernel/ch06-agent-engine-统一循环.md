...
v0.1.38 的 `run_control.rs` 实现了一套完整的运行取消原语。核心数据结构是 `ACTIVE_RUNS`——一个全局的 `(channel, chat_id) -> Vec<ActiveRun>` 映射。
...
v0.1.38 引入了 `SOUL.md` 作为人格自定义机制。`load_soul_content` 按以下优先级搜索：
...
`build_system_prompt` 是一个近 200 行的函数，它组装：

1. **身份部分**：来自 SOUL.md 或默认描述。
2. **身份规则**：如何回答"你是谁"。
3. **能力目录**：列出所有可用工具及其使用场景。
4. **权限模型**：解释 chat_id 作用域和 control chat 概念。
5. **时间上下文**：当前本地时间和 UTC，使用配置的时区。
6. **执行可靠性要求**：必须等工具返回成功才能声称完成。
7. **内置执行手册**：什么时候直接调工具、什么时候先确认。
8. **记忆上下文**：文件记忆 + 结构化记忆，带 token 预算控制。
9. **技能目录**：可用 skills 列表。
10. **插件上下文**：plugin prompt 和 document 注入。

v0.1.38 新增的关键部分包括：todo 工具的强制使用规则（"如果你要调用任何工具，必须先创建 todo list"）、子代理编排模板（depth-2 orchestration pattern）和 per-channel 系统提示词扩展。
...
v0.1.38 还在 compaction 前执行 `archive_conversation`——把原始消息归档到文件，作为不可逆压缩的安全网。compaction 的 LLM 用量会被单独记录为 `"compaction"` 类型，方便成本追踪。
...
统一循环最核心的部分是主 for 循环，最多运行 `max_tool_iterations`（默认 100）轮。每轮：
...
v0.1.38 新增了 `ToolWaveStart`/`ToolWaveComplete`（来自并行工具执行）和 `Cancelled`（来自 run_control）。这组事件服务于 Web 流式界面、SSE 和 tracing/metrics。
...
系统可能在几十轮内白白消耗大量 token 和工具成本。v0.1.38 的 fingerprint 检测在连续 6 次相同调用后即止损。
...
MicroClaw v0.1.38 的统一循环之所以重要，是因为它把原本容易分散在各处的能力——session 恢复、SOUL 人格、记忆注入、图像输入、工具循环、并行执行、compaction、审批、取消、串行化、子代理集成——收敛成了一条可恢复、可压缩、可观测、可中断的主链路。取消和串行化从"渠道特性"升级为"内核原语"，SOUL.md 从"提示词技巧"升级为"runtime 配置"。
