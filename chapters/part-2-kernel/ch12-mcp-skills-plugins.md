...
- 核心源码路径：`src/mcp.rs`、`src/skills.rs`、`src/plugins.rs`、`src/tools/mod.rs`
- 关键配置项：`src/config.rs` 中与 MCP server、Skills、Plugins、限流和熔断相关的配置
- 测试 / 运行文档路径：`README.md`（`Skills`, `Plugins`, `MCP`）；`docs/plugins/overview.md`；`docs/clawhub/overview.md`；`TEST.md`（`18. Skills System`, `19. MCP Integration`）；`docs/test/blackbox-core-20-cases.md`（`TC16 MCP 基础调用`, `TC17 MCP 限流/隔离触发`, `TC18 MCP 熔断与恢复`）
